      ...
      <div className="mt-4">
        {Array.isArray(tips) && tips.length > 0 ? (
          tips.map((tip: { id: number; content: string }) => (
            <div key={tip.id} className="flex items-start mb-2 p-2 bg-white rounded-md shadow-sm">
              <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mr-2">
                <PiggyBank className="text-green-500 text-sm" />
              </div>
              <div>
            ...
            <h3 className="text-lg font-medium text-gray-700 mb-2">아직 공유된 팁이 없어요!</h3>
            <p className="text-gray-500 text-center mb-4">첫 번째로 여러분의 유용한 팁을 공유해보세요.</p>
